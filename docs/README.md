...
AdminAssistant 是一个基于 MCP (Model Context Protocol) 的智能行政办公智能体工具集，专为中小企业和个人用户提供**通过自然语言**操作应用启动、发票识别、邮件处理、PDF文件处理、 工时统计和考勤分析等全方位管理功能。适合公司行政人员在提交公司 OA 或 ERP 系统之前要进行的一系列信息处理工作。有问题请加微信群
...
- 通过自然语言启动用户本地应用，例如『打开微信』，方便用户在行政助理中工作时直接打开其它应用

### 🧾 发票识别类

- **单个发票识别** - 识别单个发票文件，提取关键信息
- **批量发票识别** - 批量识别目录下的所有发票，并生成统计表，并复制所有发票并重命名
...
- 根据上个月的考勤信息，以及入、离职人员信息、请假信息自动生成当月考勤表，用于工资发放
...
在TRAE 中使用以下自然语言安装 Python3.12（Windows)，先用 TRAE 打开一个文件夹，通常打开用户的主目录:

* "使用 winget命令安装 Python 3.13" （如果有提示是否运行，请运行命令。中间在终端窗口中回答 Y，以确认安装。） 安装完后启启 TRAE
* "使用 winget 命令安装git"  （如果有提示是否运行，请运行命令。中间在终端窗口中回答 Y，以确认安装。） 安装完后启启 TRAE
